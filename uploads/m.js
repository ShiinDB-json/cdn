...
(function() {
    ...
    registerPlugin({
        ...
        fetchData() {
            ...
            let out = "=== PAYLOAD VIEWER ===\n\n";
            networkLog.forEach((req, idx) => {
                out += `[${idx + 1}] ${req.method} ${req.url}\n`;
                // Request body
                if (req.body) {
                    out += "Request Body:\n";
                    try {
                ...
                if (req.responseBody) {
                    out += "Response Body:\n";
                    try {
                        let resBody = req.responseBody;
                        if (typeof resBody === "string") {
                            try {
                                resBody = JSON.parse(resBody);
                                out += JSON.stringify(resBody, null, 2) + "\n";
                            } catch {
                                out += resBody + "\n";
                            }
                        } else {
                        ...
                        out += "[Error displaying response body]\n";
                    }
                } else {
                    out += "Response Body: (none)\n";
                }
                out += "\n" + "─".repeat(60) + "\n\n";
            });
            return out;
